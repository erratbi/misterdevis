...
/**
 * Class Auth_Model_PortailRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PortailRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'POR' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Portail();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Portail'] as $key => $val ) {
