...
/**
 * Class Auth_Model_PlombrieRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PlombrieRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'PLO' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Plombrie();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Plombrie'] as $key => $val ) {
