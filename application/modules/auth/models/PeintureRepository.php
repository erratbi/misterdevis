...
/**
 * Class Auth_Model_PeintureRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PeintureRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'PEI' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Peinture();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Peinture'] as $key => $val ) {
