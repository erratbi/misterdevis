...
/**
 * Class Auth_Model_ChauffageInertieRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_ChauffageInertieRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'CHI' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_ChauffageInertie();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['ChauffageInertie'] as $key => $val ) {
