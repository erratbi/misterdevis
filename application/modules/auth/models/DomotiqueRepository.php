...
/**
 * Class Auth_Model_DomotiqueRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_DomotiqueRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'DOM' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Domotique();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Domotique'] as $key => $val ) {
