...
/**
 * Class Auth_Model_ChauffagePiscineRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_ChauffagePiscineRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'CPI' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_ChauffagePiscine();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['ChauffagePiscine'] as $key => $val ) {
