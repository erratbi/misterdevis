...
/**
 * Class Auth_Model_DiagnosticRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_DiagnosticRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'DII' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Diagnostic();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Diagnostic'] as $key => $val ) {
