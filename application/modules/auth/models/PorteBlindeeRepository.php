...
/**
 * Class Auth_Model_PorteBlindeeRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PorteBlindeeRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'POB' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_PorteBlindee();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['PorteBlindee'] as $key => $val ) {
