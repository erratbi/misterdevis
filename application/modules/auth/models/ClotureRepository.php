...
/**
 * Class Auth_Model_ClotureRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_ClotureRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'CLO' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Cloture();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Cloture'] as $key => $val ) {
