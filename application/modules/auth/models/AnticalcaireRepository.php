...
/**
 * Class Auth_Model_AnticalcaireRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_AnticalcaireRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'TAC' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Anticalcaire();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Anticalcaire'] as $key => $val ) {
