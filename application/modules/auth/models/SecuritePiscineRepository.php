...
/**
 * Class Auth_Model_SecuritePiscineRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_SecuritePiscineRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'SCP' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_SecuritePiscine();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['SecuritePiscine'] as $key => $val ) {
