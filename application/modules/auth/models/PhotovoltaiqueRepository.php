...
/**
 * Class Auth_Model_PhotovoltaiqueRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PhotovoltaiqueRepository extends EntityRepository {
   ...
   * @param      $data
   *
   * @return \Auth_Model_Photovoltaique|null|object
  ...
  public function save( $id_demande = null, $data ) {
     ...
     * @var \Auth_Model_PhotovoltaiqueRepository $qualification
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'PHO' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Photovoltaique();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Photovoltaique'] as $key => $val ) {
