...
/**
 * Class Auth_Model_TerrassementRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_TerrassementRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'TER' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Terrassement();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Terrassement'] as $key => $val ) {
