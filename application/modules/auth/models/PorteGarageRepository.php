...
/**
 * Class Auth_Model_PorteGarageRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PorteGarageRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'POG' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_PorteGarage();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['PorteGarage'] as $key => $val ) {
