...
/**
 * Class Auth_Model_AlarmeIncendieRepository
 *
...
class Auth_Model_AlarmeIncendieRepository extends EntityRepository {
	...
	public function save( $id_demande = null, $data ) {
		...
		if ( ! $demande ) {
			...
			$activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'ALI' ] );
		...
		if ( ! $qualification ) {
			$qualification = new Auth_Model_AlarmeIncendie();
		}
		
		
		// Populating qualificaiton object
		foreach ( $data['AlarmeIncendie'] as $key => $val ) {
