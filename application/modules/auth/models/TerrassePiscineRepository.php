...
/**
 * Class Auth_Model_TerrassePiscineRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_TerrassePiscineRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'TRP' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_TerrassePiscine();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['TerrassePiscine'] as $key => $val ) {
