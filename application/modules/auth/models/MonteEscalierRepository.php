...
/**
 * Class Auth_Model_MonteEscalierRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_MonteEscalierRepository extends EntityRepository {
   ...
   * @param      $data
   *
   * @return \Auth_Model_MonteEscalier|null|object
  ...
  public function save( $id_demande = null, $data ) {
     ...
     * @var \Auth_Model_MonteEscalierRepository $qualification
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'MES' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_MonteEscalier();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['MonteEscalier'] as $key => $val ) {
