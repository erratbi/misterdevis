...
/**
 * Class Auth_Model_JardinParcRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_JardinParcRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'PAY' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_JardinParc();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['JardinParc'] as $key => $val ) {
