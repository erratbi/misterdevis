...
/**
 * Class Auth_Model_EntretienPiscineRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_EntretienPiscineRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'ENP' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_EntretienPiscine();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['EntretienPiscine'] as $key => $val ) {
