...
/**
 * Class Auth_Model_VoletsRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_VoletsRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'VOL' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Volets();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Volets'] as $key => $val ) {
