...
/**
 * Class Auth_Model_StationEpurationRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_StationEpurationRepository extends EntityRepository {
   ...
   * @param      $data
   *
   * @return \Auth_Model_StationEpuration|null|object
  ...
  public function save( $id_demande = null, $data ) {
     ...
     * @var \Auth_Model_StationEpurationRepository $qualification
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'MSE' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_StationEpuration();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['StationEpuration'] as $key => $val ) {
