...
/**
 * Class Auth_Model_NettoyageRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_NettoyageRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'NET' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Nettoyage();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Nettoyage'] as $key => $val ) {
