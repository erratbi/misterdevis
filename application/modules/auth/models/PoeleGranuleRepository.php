...
/**
 * Class Auth_Model_PoeleGranuleRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PoeleGranuleRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'PGR' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_PoeleGranule();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['PoeleGranule'] as $key => $val ) {
