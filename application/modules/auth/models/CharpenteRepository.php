...
/**
 * Class Auth_Model_CharpenteRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_CharpenteRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'CHP' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Charpente();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Charpente'] as $key => $val ) {
