...
/**
 * Class Auth_Model_ParquetRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_ParquetRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'PAQ' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Parquet();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Parquet'] as $key => $val ) {
