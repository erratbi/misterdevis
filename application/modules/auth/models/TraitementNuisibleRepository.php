...
/**
 * Class Auth_Model_TraitementNuisibleRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_TraitementNuisibleRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'TCN' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_TraitementNuisible();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['TraitementNuisible'] as $key => $val ) {
