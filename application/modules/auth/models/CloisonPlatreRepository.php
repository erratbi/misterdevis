...
/**
 * Class Auth_Model_CloisonPlatreRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_CloisonPlatreRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'CLP' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_CloisonPlatre();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['CloisonPlatre'] as $key => $val ) {
