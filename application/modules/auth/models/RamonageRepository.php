...
/**
 * Class Auth_Model_RamonageRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_RamonageRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'RAM' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Ramonage();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Ramonage'] as $key => $val ) {
