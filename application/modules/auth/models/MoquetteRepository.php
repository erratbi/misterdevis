...
/**
 * Class Auth_Model_MoquetteRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_MoquetteRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'MOQ' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Moquette();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Moquette'] as $key => $val ) {
