...
/**
 * Class Auth_Model_InterphoneRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_InterphoneRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'INT' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Interphone();
    }
    
    
    // Populating qualificaiton object
    // foreach ( $data['AbriJardin'] as $key => $val ) {
