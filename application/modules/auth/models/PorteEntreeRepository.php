...
/**
 * Class Auth_Model_PorteEntreeRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_PorteEntreeRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'POE' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_PorteEntree();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['PorteEntree'] as $key => $val ) {
