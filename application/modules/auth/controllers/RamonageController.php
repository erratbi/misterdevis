...
/**
 * Class Auth_RamonageController
 *
 * @authors  Youssef Erratbi <yerratbi@gmail.com>  - Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date     23/12/17
 * Ce controlleur est responsable sur la gestion de l'activité ramonage,
 * il permet de lister les demande de devis concernant le ramonage avec l'action indexAction,
 ...
 */
class Auth_RamonageController extends Zend_Controller_Action {
  
  private $_sys_email;
  private $type       = 'RAM';
  private $slug       = 'ramonage';
  private $name       = 'Ramonage';
  private $form_name  = 'Auth_Form_Ramonage';
  private $model_name = 'Auth_Model_Ramonage';
  ...
  public function editAction() {
    ...
    if ( $this->getRequest()->isPost() ) {
      ...
      if ( isset( $_FILES['audio_file'] ) ) {
        $filename = APPLICATION_PATH . '/../public/pdf/' . basename( $_FILES['audio_file']['name'] );
        move_uploaded_file( $_FILES['audio_file']['tmp_name'], $filename );
      }
      ...
      if ( isset( $_FILES['audio_file'] ) && $_FILES['audio_file']['type'] && ! in_array( $_FILES['audio_file']['type'], [ 'audio/wav', 'audio/x-wav', 'audio/mpeg', 'application/ogg', 'text/plain' ] ) ) {
        $valid = false;
