...
class Auth_PiscineController extends Zend_Controller_Action {
  ...
  private function sendEmailNotifications( $demande ) {
    ...
    $artisans1 = $em->getRepository( 'Auth_Model_Artisan' )->findListEmail(
      $demande->getId_activite()->getId_activite(),
      $demande->getId_chantier()->getZone()->getCode_departement()
    );
    
    
    $artisans2 = $em->getRepository( 'Auth_Model_Artisan' )->findListEmailByGeo(
      $demande->getId_activite()->getId_activite(),
      $demande->getId_chantier()->getZone()->getId_zone()
    );



    $artisans = array_merge( $artisans1, $artisans2 );
    ...
    //Send Email To Externe Artisans

    $artisanExterne = $em->getRepository('Auth_Model_Email')->findListEmail(
      $this->type,
      $demande->getId_chantier()->getZone()->getCode_departement()
    );
    

    foreach ($artisanExterne as $artisan) {

       try {  

          $html = $this->view->partial( 'shared/mail_masse_artisan.phtml', ['demande' => $demande]);
          ...
          $mail->setSubject( "Demande de devis - ".$demande->getId_activite()->getLibelle()." sur ".ucfirst(strtolower($demande->getId_chantier()->getZone()->getVille())) . " (".$demande->getId_chantier()->getZone()->getCode().")" );
          $mail->addTo( $artisan['email'],"");
    ...
    // Send SMS To Externe Artisans

    $artisanExterne = $em->getRepository('Auth_Model_Sms')->findListPhone(
      $this->type,
      $demande->getId_chantier()->getZone()->getCode_departement()
    );

    if(!empty($artisanExterne)){
      $this->sendSMSExterne($artisanExterne);
    }

  }

  public function sendSMSExterne($artisans){

    $sms = new smsenvoi();

    foreach ($artisans as $artisan) {

      $content="Bonjour,\nFantastique, 1 nouveau Chantier est maintenant disponible pour votre activité et dans votre secteur.\n
                  Vous pouvez le consulter sans engagement en vous inscrivant sur notre site <a href=\"https://www.mister-devis.com/inscription-pro/\">https://www.mister-devis.com/inscription-pro/</a>\n
                    0988772700\n
                    STOP";
  ...
  public function sendSMSNotification( $artisans, $ref ) {
    ...
    $content = "Bonjour,\nFantastique, 1 nouveau Chantier est maintenant disponible dans votre Espace Pro et nous vous invitons à le consulter dans les meilleurs délais.\nDemande N° {$ref}\n<a href=\"http://pro.mister-devis.com\">Espace Pro</a>";
  ...
  public function editAction() {
    ...
    $form->form_demande->setDefaults( [ 'titre_demande' => $demande->getRef() ] );
    $form->form_chantier->setDefaults( [ 'code_postal' => $demande->id_chantier->zone->code ] );
    $form->form_chantier->setDefaults( [ 'ville' => $demande->id_chantier->zone->ville ] );
    
    // Proccess the posted data;
    if ( $this->getRequest()->isPost() ) {
      $data = $this->getRequest()->getPost();
      $zone = $em->getRepository( 'Auth_Model_Zone' )->find( $data['Chantier']['id_zone'] );
